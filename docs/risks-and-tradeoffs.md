...
| Worker lag during traffic spike | Bounded retry + back-pressure via Redis stream length cap (`MAXLEN ~ 1_000_000`) on `XADD` | If logs are arriving faster than we can write, oldest are dropped at the stream level rather than backing up forever. Surfaced as a metric. |
